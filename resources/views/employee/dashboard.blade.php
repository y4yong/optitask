...
@section('content')
    <header class="flex justify-between items-end mb-6">
        <div>
            <h1 class="text-3xl font-extrabold text-[#1e293b] tracking-tight uppercase">Dashboard</h1>
            <p class="text-pink-400 mt-1 font-bold text-sm">Welcome back, {{ explode(' ', trim(Auth::user()->username))[0] }}.</p>
        </div>
    </header>

    @if(session('success'))
        <div class="mb-4 p-4 rounded-2xl text-center text-sm font-semibold bg-green-50 text-green-500">
            {{ session('success') }}
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="glass-card overflow-hidden bg-white">
                <div class="px-5 py-4 border-b border-pink-50 bg-white/55 flex justify-between items-center">
                    <h3 class="font-extrabold text-[#1e293b] text-base tracking-tight flex items-center gap-2">
                        <span class="w-2 h-5 bg-[#FB6F92] rounded-full shrink-0"></span>
                        Active Assignments
                    </h3>
                ...
                @if($activeTasks->count() > 0)
                    {{-- Mobile card view --}}
                    <div class="lg:hidden divide-y divide-pink-50">
                        @foreach($activeTasks as $row)
                        <div class="p-4 space-y-2">
                            <div class="flex items-start justify-between gap-2">
                                <div class="min-w-0">
                                    <p class="text-sm font-extrabold text-[#1e293b] leading-tight">{{ $row->task_title }}</p>
                                    <p class="text-[10px] font-mono text-slate-400 mt-0.5">{{ $row->task_id }}</p>
                                </div>
                                <span class="shrink-0 px-2 py-0.5 rounded text-[9px] font-black uppercase {{ $row->task_status === 'In Progress' ? 'bg-amber-50 text-amber-600 border border-amber-100' : ($row->task_status === 'Done' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-pink-50 text-[#FB6F92] border border-pink-100') }}">
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-1.5 text-xs font-bold text-gray-400">
                                    ...
                                    @if(in_array($row->task_status, ['To-Do', 'In Progress']) && strtotime($row->due_date) < strtotime(date('Y-m-d')))
                                        <span class="px-1.5 py-0.5 rounded text-[8px] font-black uppercase bg-red-50 text-red-500 border border-red-100 animate-pulse">Overdue</span>
                                    @endif
                                </div>
                                <a href="{{ route('employee.tasks') }}" class="inline-flex items-center gap-1 bg-pink-50 hover:bg-[#FB6F92] text-[#FB6F92] hover:text-white px-3 py-1.5 rounded-lg text-[10px] font-black transition-all">
                            ...
                            <tbody class="divide-y divide-pink-50">
                                ...
                                <tr class="hover:bg-[#FFF9FA] transition-all">
                                    ...
                                    <td class="px-6 py-5 text-right">
                                        <a href="{{ route('employee.tasks') }}" class="inline-flex items-center gap-1.5 bg-pink-50 hover:bg-[#FB6F92] text-[#FB6F92] hover:text-white px-4 py-2 rounded-xl text-xs font-black transition-all shadow-sm">
                                            <i data-lucide="external-link" class="w-3.5 h-3.5"></i> Task Details
                                        </a>
                                    </td>
                                </tr>
                                @endforeach
                ...
                @else
                    <div class="py-8 text-center">
                        <div class="w-10 h-10 bg-pink-50 rounded-2xl flex items-center justify-center mx-auto mb-2">
                            <i data-lucide="inbox" class="w-5 h-5 text-pink-200"></i>
                        </div>
            ...
            <div class="glass-card overflow-hidden bg-white">
                <div class="px-5 py-4 border-b border-pink-50 bg-white/55 flex justify-between items-center">
                    <h3 class="font-extrabold text-[#1e293b] text-base tracking-tight flex items-center gap-2">
                        <span class="w-2 h-5 bg-blue-500 rounded-full shrink-0"></span>
                        Verified Assignments
                    </h3>
                ...
                @if($verifiedTasks->count() > 0)
                    ...
                    <div class="lg:hidden divide-y divide-pink-50">
                        ...
                        <div class="p-4 flex items-center justify-between gap-3">
                            <div class="min-w-0">
                                <p class="text-sm font-extrabold text-[#1e293b] truncate">{{ $row->task_title }}</p>
                                <div class="flex items-center gap-1.5 mt-1 text-xs font-bold text-gray-400">
                                    <i data-lucide="calendar-check" class="w-3 h-3 text-blue-400"></i>
                                    {{ date('M d, Y', strtotime($row->updated_at)) }}
                                </div>
                            </div>
                            <span class="shrink-0 inline-flex items-center gap-1 bg-blue-50 text-blue-600 px-2.5 py-1 rounded-lg text-[9px] font-black border border-blue-100 uppercase tracking-wider">
                        ...
                        <table class="w-full text-left">
                            <thead>
                                <tr class="text-[11px] uppercase font-black text-pink-300 tracking-[0.1em] border-b border-pink-50">
                                    <th class="px-6 py-4">Task ID</th>
                                    <th class="px-6 py-4">Project Details</th>
                                    <th class="px-6 py-4">Completed Date</th>
                ...
                @else
                    <div class="py-8 text-center">
                        <div class="w-10 h-10 bg-pink-50 rounded-2xl flex items-center justify-center mx-auto mb-2">
                            <i data-lucide="award" class="w-5 h-5 text-pink-200"></i>
                        </div>
        ...
        {{-- Sidebar widgets --}}
        <div class="space-y-5">
            <div class="glass-card p-6 flex flex-col items-center text-center bg-white">
                <h4 class="text-[11px] font-black text-pink-300 uppercase tracking-widest mb-5">Efficiency Rating</h4>
                
                <div class="relative w-36 h-36 flex items-center justify-center mb-4">
                    <svg class="w-36 h-36 transform -rotate-90 absolute" viewBox="0 0 36 36">
            ...
            <div class="pink-gradient p-6 rounded-[2rem] text-white shadow-xl shadow-pink-200/50 relative overflow-hidden">
                <i data-lucide="sparkles" class="absolute -right-4 -bottom-4 w-28 h-28 opacity-10 rotate-12"></i>
                <h4 class="text-xs font-black uppercase tracking-[0.2em] mb-3 opacity-80">Efficiency Insight</h4>
                <div class="flex items-center justify-between mb-3">
                    <span class="text-2xl font-extrabold italic tracking-tight">AI Coach</span>
                    <i data-lucide="cpu" class="w-9 h-9 opacity-50"></i>
                </div>
